<template><div><p>最近身边学习 Vue 的人也越来越多了，大家都有很多的疑问，为什么前端要用框架、Vue 做了些什么、要怎么上手，等等。距离上一次写 Vue 也过去三年了，是时候再把这三年自己的理解再整理进去了。由于这个系列主要是希望写给可能前端掌握也不深的人，所以会铺垫一些自己认为需要的内容，本节先来讲讲入门前端，作为铺垫，来理解 Vue 这个框架吧。</p>
<p>最近根据之前收集大家的一些对前端和 Vue 的疑问，整理了个科普文章，内容包括：</p>
<ol>
<li>HTML/CSS/JS 职责，浏览器如何解析</li>
<li>浏览器是怎么渲染页面的</li>
<li>Vue 这类框架都做了什么（前端模版引擎简介）</li>
<li>Vue 与传统 jQuery 区别（理解数据驱动与事件驱动）</li>
<li>前端工程化的简单介绍</li>
</ol>
<!--more-->
<h1 id="前端页面解析" tabindex="-1"><a class="header-anchor" href="#前端页面解析" aria-hidden="true">#</a> 前端页面解析</h1>
<h2 id="页面组成" tabindex="-1"><a class="header-anchor" href="#页面组成" aria-hidden="true">#</a> 页面组成</h2>
<p>我们打开一个前端项目，经常会看到很多不同后缀的文件，例如一个页面可能包括<code v-pre>a.html</code>、<code v-pre>a.css</code>、<code v-pre>a.js</code>，用了 Vue 还有<code v-pre>a.vue</code>，再加上 Typescript 可能还有<code v-pre>a.ts</code>。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里面包括两个子模快：</p>
<ul>
<li><code v-pre>&lt;head&gt;</code>：常包括控制样式的<code v-pre>&lt;link&gt;</code>标签、控制浏览器特殊逻辑的<code v-pre>&lt;meta&gt;</code>标签、控制代码执行逻辑的<code v-pre>&lt;script&gt;</code>，不展示到页面。</li>
<li><code v-pre>&lt;body&gt;</code>：包括展示在页面的内容。</li>
</ul>
<p>通常来说，一段 HTML 代码，最终在浏览器中会生成一堆 DOM 节点树，例如：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这段代码在浏览器中渲染时，其实是长这个样子的：
<img src="https://github-imglib-1255459943.cos.ap-chengdu.myqcloud.com/wxapp-3.png" alt=""></p>
<p>这不一定是最终的结果，因为我们还可以这样操作：</p>
<ol>
<li>通过 CSS 样式，例如<code v-pre>display: none</code>来让其中某个节点消失。</li>
<li>通过 JS 代码操作 DOM，例如使用<code v-pre>document.getElementById</code>获取到某个节点元素，然后再通过设置<code v-pre>.innerHTML</code>来改变元素的内容。</li>
</ol>
<h3 id="css" tabindex="-1"><a class="header-anchor" href="#css" aria-hidden="true">#</a> CSS</h3>
<p>CSS 主要是给我们的 HTML 元素添加样式，可以通过几个方式匹配:</p>
<ul>
<li>DOM 元素匹配：例如<code v-pre>p{color: red}</code>会让所有<code v-pre>&lt;p&gt;</code>元素的文字都变成红色</li>
<li>class 匹配：类的匹配，例如<code v-pre>.color-red{color: red}</code>会让所有<code v-pre>&lt;xxx class=&quot;color-red&quot;&gt;</code>元素的文字都变成红色</li>
<li>id 匹配：id 标识符的匹配，例如<code v-pre>#color-red{color: red}</code>会让<code v-pre>&lt;xxx id=&quot;color-red&quot;&gt;</code>元素的文字都变成红色（若页面内有多个相同的 id，则只有第一个生效）</li>
</ul>
...
<p>HTML 是简单的网页静态信息，而 JavaScript 可以在网页上实现复杂的功能。</p>
...
<p>Javascript 是单线程的，更多是因为对页面交互的同步处理。作为浏览器脚本语言，JavaScript 的主要用途是与用户互动，以及操作 DOM，若是多线程会导致严重的同步问题。</p>
<p>关于更多的 Javascript，可以阅读<RouterLink to="/front-end-basic/front-end/front-end-5.html">《5. Javascript》</RouterLink>。</p>
<h2 id="页面渲染" tabindex="-1"><a class="header-anchor" href="#页面渲染" aria-hidden="true">#</a> 页面渲染</h2>
<h3 id="浏览器的渲染机制" tabindex="-1"><a class="header-anchor" href="#浏览器的渲染机制" aria-hidden="true">#</a> 浏览器的渲染机制</h3>
<p>我们现在知道一个页面的代码里，主要包括了 HTML、CSS、Javascript 三大块内容，那么浏览器是怎么解析和加载这些内容的呢？</p>
...
<p>CSS 规则树与 DOM 结构树结合，最终生成一个 Render 树（即最终呈现的页面，例如其中会移除 DOM 结构树中匹配到 CSS 里面<code v-pre>display:none</code>的 DOM 节点）。一般来说浏览器绘制页面的过程是：</p>
<ol>
...
<p>更多的资料，大家可以参考<a href="https://coolshell.cn/articles/9666.html" target="_blank" rel="noopener noreferrer">《浏览器的渲染原理简介》<ExternalLinkIcon/></a>，或者英文很棒的你也可以阅读原文<a href="http://taligarsiel.com/Projects/howbrowserswork1.htm" target="_blank" rel="noopener noreferrer">《How browsers work》<ExternalLinkIcon/></a></p>
<h3 id="页面的局部刷新" tabindex="-1"><a class="header-anchor" href="#页面的局部刷新" aria-hidden="true">#</a> 页面的局部刷新</h3>
<p>一般看来，浏览器生成了最终的 Render 树，页面也已经渲染完毕，似乎浏览器已经完成了它的工作了。但现实中我们的页面更多的不只是静态的页面，还会包括点击、拖拽等事件操作，以及接口请求、数据渲染到页面等动态的交互逻辑，这时候我们会需要更新页面的信息。</p>
<p>我们的业务代码中情况会复杂得多，除了插入内容，还包括内容更新、删除元素节点等。不管是那种情况，目前来说前端一般分为两种方式：</p>
<ol>
<li>绑定映射表方式。</li>
<li>直接替换内容方式。</li>
</ol>
<h4 id="_1-绑定映射表方式" tabindex="-1"><a class="header-anchor" href="#_1-绑定映射表方式" aria-hidden="true">#</a> 1. 绑定映射表方式</h4>
<p>这其实是挺经常使用的一种方式，例如下面这段代码：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这里拿到了<code v-pre>&lt;xxx id=&quot;a&quot;&gt;</code>的这样一个元素映射，我们在更新内容、处理节点的时候就可以用这个映射来直接操作，如：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 1. 更改元素里面内容</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果我们一个页面里需要绑定变量的元素很多，那每次要更新某块的页面数据，可能会需要保存很多的元素映射，同时需要调用很多很多的<code v-pre>createElement()/appendChild()/removeChild()</code>这类方法，也是个不小的开销。这种情况下，我们可以使用直接替换内容的方式。</p>
<h4 id="_2-直接替换内容方式" tabindex="-1"><a class="header-anchor" href="#_2-直接替换内容方式" aria-hidden="true">#</a> 2. 直接替换内容方式</h4>
<p>我们每次更新页面数据和状态，还可以通过<code v-pre>innerHTML</code>方法来用新的<code v-pre>HTML String</code>替换旧的，这种方法写起来很简单，无非是将各种节点使用字符串的方式拼接起来而已。</p>
<p>例如，上面的几次更新 a 元素节点，可以调整成这样实现：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 1. 更改元素里面内容</span>
a<span class="token punctuation">.</span>innerHTML <span class="token operator">=</span> <span class="token string">"&lt;p>测试&lt;/p>"</span><span class="token punctuation">;</span>
<span class="token comment">// 2. 插入一个&lt;a>元素</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这种方式来更新页面简单粗暴，但是如果我们更新的节点范围比较大，这时候我们需要替换掉很大一片的<code v-pre>HTML String</code>。这种情况下，会面临着可能导致更多的浏览器计算。</p>
<h3 id="页面回流、重绘" tabindex="-1"><a class="header-anchor" href="#页面回流、重绘" aria-hidden="true">#</a> 页面回流、重绘</h3>
<p>前面也介绍了，浏览器绘制页面的过程是：1.计算<code v-pre>CSS规则树</code> =&gt; 2.生成<code v-pre>Render树</code> =&gt; 3.计算各个节点的<code v-pre>大小</code>/<code v-pre>position</code>/<code v-pre>z-index</code> =&gt; 4.绘制。其中计算的环节也是消耗较大的地方。</p>
<p>我们使用 DOM API 和 CSS API 的时候，通常会触发浏览器的两种操作：Repaint（重绘） 和 Reflow（回流）：</p>
<ul>
<li><strong>Repaint：页面部分重画，通常不涉及尺寸的改变，常见于颜色的变化。</strong> 这时候一般只触发绘制过程的第 4 个步骤。</li>
<li><strong>Reflow：意味着节点需要重新计算和绘制，常见于尺寸的改变。</strong> 这时候会触发 3 和 4 两个步骤。<br>
在 Reflow 的时候，浏览器会使渲染树中受到影响的部分失效，并重新构造这部分渲染树，完成 Reflow 后，浏览器会重新绘制受影响的部分到屏幕中，该过程成为 Repaint。</li>
</ul>
<p>回流的花销跟 render tree 有多少节点需要重新构建有关系，这也是为什么前面说使用<code v-pre>innerHTML</code>会导致更多的开销。所以到底是使用绑定映射表方式，还是使用直接替换内容方式，都是需要具体问题具体分析的。</p>
<h2 id="事件驱动" tabindex="-1"><a class="header-anchor" href="#事件驱动" aria-hidden="true">#</a> 事件驱动</h2>
<p>事件驱动其实是前端开发中最容易理解的编码方式，例如我们写一个提交表单的页面，用事件驱动的方式来写的话，会是这样一个流程：</p>
<ol>
<li>编写静态页面。</li>
</ol>
<div class="language-html line-numbers-mode" data-ext="html"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>form</span><span class="token punctuation">></span></span>
  Name:
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>以上这个流程，是很常见的前端编码思维，我们称之为 <strong>事件驱动</strong> 模式。</p>
<h1 id="前端思维转变" tabindex="-1"><a class="header-anchor" href="#前端思维转变" aria-hidden="true">#</a> 前端思维转变</h1>
<p>很多人不理解这几年来前端的变化，为什么不能再用 jQuery 统一天下呢？为什么要搞那么多的库，还要按照环境呢？不是用个 txt 编辑器就能写完一个页面吗，前端弄那么复杂是为了什么呢？</p>
<p>既然称之为思维转变，那么可以将事件驱动的思维模式作为过去常见的一种方式，而如今的前端开发过程中，多了很多的新框架、新工具，还有了工程化，这些带来了怎样的思维模式的转变呢？</p>
<h2 id="前端框架的出现" tabindex="-1"><a class="header-anchor" href="#前端框架的出现" aria-hidden="true">#</a> 前端框架的出现</h2>
<p>其实最初是 AngularJS 开始占领了比较多的地位，后面 React 迎面赶上，在 Angular 断崖升级的帮助下，Vue 结合了各种框架的优势，以及非常容易入门的文档，成功成为了那一匹黑马。既然这一系列写的是 Vue 的入门和使用，那这里当然是基于 Vue 来介绍了。</p>
<h3 id="数据绑定" tabindex="-1"><a class="header-anchor" href="#数据绑定" aria-hidden="true">#</a> 数据绑定</h3>
<h4 id="vue-文本插值" tabindex="-1"><a class="header-anchor" href="#vue-文本插值" aria-hidden="true">#</a> Vue 文本插值</h4>
<p>在 Vue 中，最基础的模板语法是数据绑定，例如：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这里绑定了一个 msg 的变量，开发者在 Vue 实例 data 中绑定该变量：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">new</span> <span class="token class-name">Vue</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  <span class="token literal-property property">data</span><span class="token operator">:</span> <span class="token punctuation">{</span>
    <span class="token literal-property property">data</span><span class="token operator">:</span> <span class="token string">"测试文本"</span>
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最终页面展示内容为<code v-pre>&lt;div&gt;测试文本&lt;/div&gt;</code>。</p>
<h4 id="数据绑定的实现" tabindex="-1"><a class="header-anchor" href="#数据绑定的实现" aria-hidden="true">#</a> 数据绑定的实现</h4>
<p>这种使用双大括号来绑定变量的方式，我们称之为数据绑定。它是怎么实现的呢，数据绑定的过程其实不复杂：</p>
<ol>
<li>解析语法生成 AST。</li>
<li>根据 AST 结果生成 DOM。</li>
<li>将数据绑定更新至模板。</li>
</ol>
<p>上述这个过程，是模板引擎在做的事情。我们来看看上面在 Vue 里的代码片段<code v-pre>&lt;div&gt;{{ data }}&lt;/div&gt;</code>，我们可以解析后获得这样一个 AST 对象：</p>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 假设这是一个生成 DOM 的过程，包括 innerHTML 和事件监听</span>
...
<span class="token keyword">function</span> <span class="token function">getDOMString</span><span class="token punctuation">(</span><span class="token parameter">domObj</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>关于模板引擎更多的情况，可以查看<RouterLink to="/front-end-basic/understanding/template-engine.html">《前端模板引擎》</RouterLink>。</p>
<h3 id="虚拟-dom" tabindex="-1"><a class="header-anchor" href="#虚拟-dom" aria-hidden="true">#</a> 虚拟 DOM</h3>
<p>虚拟 DOM 如今已经被作为前端面试基础题了，多多少少面试者都要去了解下，当初 React 就是因为虚拟 DOM 的提出，暂时赢下了浏览器性能之争。当然，这都是几年前的事情了，如今几大框架的性能问题，早也不是什么大问题了。</p>
<p>虚拟 DOM 大概是这么个过程：<br>
(1) 用 JS 对象模拟 DOM 树，得到一棵虚拟 DOM 树。<br>
(2) 当页面数据变更时，生成新的虚拟 DOM 树，比较新旧两棵虚拟 DOM 树的差异。<br>
(3) 把差异应用到真正的 DOM 树上。</p>
...
<p>如图，这里我们对比两棵 DOM 树，得到的差异有：p 元素插入了一个 span 元素子节点，然后原先的文本节点挪到了 span 元素子节点下面，最后通过 JS 操作就可以实现完。</p>
<h3 id="xss-漏洞" tabindex="-1"><a class="header-anchor" href="#xss-漏洞" aria-hidden="true">#</a> XSS 漏洞</h3>
<p>模板引擎还可以协助预防下 XSS 相关漏洞。我们知道，XSS 的整个攻击过程大概为：</p>
<ol>
...
<p>避免 XSS 的方法之一主要是将用户所提供的内容进行过滤，而大多数模板引擎会自带 HTML 转义功能。在 Vue 中，默认的数据绑定方式（双大括号、<code v-pre>v-bind</code>等）会进行 HTML 转义，将数据解释为普通文本，而非 HTML 代码。</p>
<p>当然，如果你一定要输出 HTML 代码，也可以使用<code v-pre>v-html</code>指令输出。<a href="https://cn.vuejs.org/v2/guide/syntax.html#%E5%8E%9F%E5%A7%8B-HTML" target="_blank" rel="noopener noreferrer">官方文档<ExternalLinkIcon/></a>也有特殊说明：</p>
<blockquote>
<p>你的站点上动态渲染的任意 HTML 可能会非常危险，因为它很容易导致 XSS 攻击。请只对可信内容使用 HTML 插值，绝不要对用户提供的内容使用插值。</p>
</blockquote>
<p>关于 XSS 漏洞，更多的可以查看<a href="https://tech.meituan.com/2018/09/27/fe-security.html" target="_blank" rel="noopener noreferrer">《前端安全系列（一）：如何防止 XSS 攻击？》<ExternalLinkIcon/></a>一文。</p>
<h3 id="vue-简介" tabindex="-1"><a class="header-anchor" href="#vue-简介" aria-hidden="true">#</a> Vue 简介</h3>
<p>看看这些模板引擎都做了啥事，原本就是一个<code v-pre>&lt;div&gt;</code>，经过 AST 生成一个对象，最终还是生成一个<code v-pre>&lt;div&gt;</code>，这是多余的步骤吗？不是的，在这个过程中我们可以实现一些功能：</p>
<ul>
...
<p>关于 Vue 和其他框架的对比，可以看看官方文档-<a href="https://cn.vuejs.org/v2/guide/comparison.html" target="_blank" rel="noopener noreferrer">对比其他框架<ExternalLinkIcon/></a>。易于上手这块，是大多数人都比较认可的，框架的性能也不错，这也是技术选型中比较重要的一些考虑。</p>
<h2 id="数据驱动" tabindex="-1"><a class="header-anchor" href="#数据驱动" aria-hidden="true">#</a> 数据驱动</h2>
<p>前面也介绍了，在 jQuery 年代，我们通常是使用事件驱动的模式去进行开发了。那么使用了 Vue 之后，写代码的方式会有哪些不一样吗？</p>
<p>既然前面介绍了事件模型一般的编码流程，我们再来看看，同样的们写一个提交表单的页面，用数据驱动的方式来写的话，会变成这么一个流程：</p>
<ol>
<li>设计数据结构。</li>
</ol>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 包括一个 name 和 一个 email 的值</span>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们在设置数据（<code v-pre>this.name = event.target.value</code>）的时候，Vue 已经替我们把将数据更新到页面的逻辑处理了。大家也可以去 <a href="https://codepen.io/anon/pen/qzPbWQ" target="_blank" rel="noopener noreferrer">codepen<ExternalLinkIcon/></a> 或者 jsfiddle、<a href="https://stackblitz.com/" target="_blank" rel="noopener noreferrer">stackblitz<ExternalLinkIcon/></a> 这些在线编码网站上尝试下。</p>
<p>所以事件驱动和数据驱动一个很重要的区别在于，我们是从每个事件的触发开始设计我们的代码，还是以数据为中心，接收事件触发和更新数据状态的方式来写代码。</p>
<h3 id="页面编写" tabindex="-1"><a class="header-anchor" href="#页面编写" aria-hidden="true">#</a> 页面编写</h3>
<p>如果要问，vue 和 jQuery 有什么不一样？其实它们从定位到设计到实现上都完全不一样，但是对开发者来说，我们可以做个简单直观的区别：</p>
<div class="language-html line-numbers-mode" data-ext="html"><pre v-pre class="language-html"><code><span class="token comment">&lt;!--1. jQuery + 事件驱动--></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当我们在 Vue 中，模板引擎帮我们处理了模板渲染、数据绑定的过程，我们只需要知道这里面只有一个有效数据，即输入框的值。</p>
<h3 id="页面抽象" tabindex="-1"><a class="header-anchor" href="#页面抽象" aria-hidden="true">#</a> 页面抽象</h3>
<p>既然使用了数据驱动，那么对页面的一个抽象能力也是很重要的。例如我们现在要写一个列表，数据从后台获取到之后，展示到页面中。</p>
<ol>
<li>当我们需要渲染成列表时：</li>
</ol>
<div class="language-html line-numbers-mode" data-ext="html"><pre v-pre class="language-html"><code><span class="token comment">&lt;!--1). jQuery + 事件驱动--></span>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 1). jQuery + 事件驱动</span>
<span class="token keyword">const</span> dom <span class="token operator">=</span> <span class="token function">$</span><span class="token punctuation">(</span><span class="token string">"#ul"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在使用数据驱动的时候，模板渲染的事情会交给框架去完成，我们需要做的就是数据处理而已。那么转换了数据驱动之后，有什么好处呢？</p>
<p>当我们把一个页面抽象成数据来表示之后，可以做的事情就多了。如果我们把所有的页面、组件、展示内容和接口配置等都变成了配置数据，那么我们就可以把所有的东西都进行配置化。以前也写过一篇<RouterLink to="/front-end-basic/understanding/component-with-configuration.html">《组件配置化》</RouterLink>，另外在玩 Angular 的时候也做过类似的设计，包括<a href="http://angular2-form-schame.godbasin.com/index.html" target="_blank" rel="noopener noreferrer">组件的配置--通过配置生成表单<ExternalLinkIcon/></a>和<a href="http://angular-custom-app.godbasin.com/#/home/custom-app" target="_blank" rel="noopener noreferrer">页面应用的配置--通过配置生成管理端<ExternalLinkIcon/></a>。当然，Vue 也完全可以做这种事情的，配置化的难度常常不在于技术有多难，而在于怎么把业务进行适当的抽象。</p>
<h2 id="前端工程化" tabindex="-1"><a class="header-anchor" href="#前端工程化" aria-hidden="true">#</a> 前端工程化</h2>
<p>其实前端工程化这块，我并不是能理解得很深刻，能讲给大家的，应该只是自己的一种理解了。一般来说，现在的主流构建工具应该是 Webpack，包括我们使用 Vue 官方脚手架生成的代码，构建工具也是 Webpack。</p>
...
<h3 id="npm-依赖包" tabindex="-1"><a class="header-anchor" href="#npm-依赖包" aria-hidden="true">#</a> npm 依赖包</h3>
<p>要实现工程化，前端开发几乎都离不开 nodejs 的包管理器 npm，比如前端在搭建本地开发服务以及打包编译前端代码等都会用到。在前端开发过程中，经常用到<code v-pre>npm install</code>来安装所需的依赖。</p>
<p>为什么会有 npm 包的出现呢？因为一个人的力量是有限的，我们在写代码的时候，为了可以将注意力集中到业务开发中，会需要使用别人开源的很多工具、框架和代码包。在很早以前，我们是一个个地下载，或是通过<code v-pre>&lt;script src=&quot;https://cdn.jsdelivr.net/npm/vue/dist/vue.js&quot;&gt;&lt;/script&gt;</code>的方式去引用。</p>
...
<p>在安装 Node.js 的时候，npm 的能力也会一块提供到，按照完之后就能直接在机器终端使用<code v-pre>npm install xxx</code>这种能力啦，需要安装什么依赖包呢，你可以去<a href="https://www.npmjs.com/" target="_blank" rel="noopener noreferrer">npmjs 官网<ExternalLinkIcon/></a>搜一下，也可以自己发布一些包供别人使用，例如之前我也写过一个<a href="https://www.npmjs.com/package/v-select2-component" target="_blank" rel="noopener noreferrer">v-select2-component<ExternalLinkIcon/></a>组件。</p>
<h3 id="vue-cli" tabindex="-1"><a class="header-anchor" href="#vue-cli" aria-hidden="true">#</a> vue cli</h3>
<p>通常来说，脚手架可以让你快速地生成示例代码、搭建本地环境，也可以更新依赖的版本等，避免了每个开发者自行调整开发环境、打包逻辑等配置。</p>
<p>Vue cli 也提供了这样的能力，更多的可以参考<a href="https://cli.vuejs.org/zh/" target="_blank" rel="noopener noreferrer">官方文档<ExternalLinkIcon/></a>。</p>
<blockquote>
<p>Vue CLI 致力于将 Vue 生态中的工具基础标准化。它确保了各种构建工具能够基于智能的默认配置即可平稳衔接，这样你可以专注在撰写应用上，而不必花好几天去纠结配置的问题。</p>
</blockquote>
<p>使用方式很简单：</p>
<div class="language-cmd line-numbers-mode" data-ext="cmd"><pre v-pre class="language-cmd"><code>// 安装脚手架
npm install -g @vue/cli
// 脚手架生成 vue 项目，同时会自动安装依赖
vue create vue-cli-demo
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>生成之后的代码目录是这样的：
<img src="https://github-imglib-1255459943.cos.ap-chengdu.myqcloud.com/vue-cli-demo-files.jpg" alt=""></p>
<h3 id="启动项目" tabindex="-1"><a class="header-anchor" href="#启动项目" aria-hidden="true">#</a> 启动项目</h3>
<p>一般来说，项目会有个 README.md 文件，你能看到一些简单的说明，例如这里生成的：</p>
<div class="language-cmd line-numbers-mode" data-ext="cmd"><pre v-pre class="language-cmd"><code># Project setup
yarn install
...
yarn run lint
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>yarn 跟 npm 都是差不多的包管理器，区别在于 yarn 在安装时会速度更快（并行、离线等），以及版本统一管理的比较好。但如果你不是有特殊的喜好或者习惯，其实两个都可以用，例如这里的<code v-pre>yarn run serve</code>也可以用<code v-pre>npm run serve</code>来运行。</p>
<p>如果有些习惯不好的项目缺了 README，这种时候要怎么去启动一个项目呢？可以查看<code v-pre>package.json</code>文件：</p>
<div class="language-json line-numbers-mode" data-ext="json"><pre v-pre class="language-json"><code><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>一般来说，开发环境是<code v-pre>dev</code>、<code v-pre>serve</code>等，生产环境是<code v-pre>build</code>，<code v-pre>scripts</code>里是一些任务，运行命令<code v-pre>npm run taskName</code>就可以启动了。
<img src="https://github-imglib-1255459943.cos.ap-chengdu.myqcloud.com/vue-cli-demo-serve.jpg" alt=""></p>
<p>上图中，我们可以看到任务已经启动了，访问输出的地址（这里是<code v-pre>http://localhost:8080/</code>或者<code v-pre>http://10.40.120.53:8080/</code>）就能看到我们的项目跑起来了。
<img src="https://github-imglib-1255459943.cos.ap-chengdu.myqcloud.com/vue-cli-demo-page.jpg" alt=""></p>
<h2 id="结束语" tabindex="-1"><a class="header-anchor" href="#结束语" aria-hidden="true">#</a> 结束语</h2>
<p>第一 part 结束了，更多的内容还是前端相关。这些主要是这几年对页面渲染的一些新的理解，然后简单用脚手架启动了个 demo 项目。内容是按照自己觉得较清晰的逻辑展开来讲的，如果说你有更好的想法，或是发现我的描述中有哪些不到位的地方，也十分欢迎来交流。<br>
工具始终是工具没错，但一个工具为什么受这么多人追捧，它到底解决了什么，你可以从中学习到什么，这些才是个人认为的在使用工具时候收获的很重要的东西。在 Vue 官方文档很完善的情况下，我来给你们补上文档以外的一些技巧和内容吧。</p>
</div></template>
