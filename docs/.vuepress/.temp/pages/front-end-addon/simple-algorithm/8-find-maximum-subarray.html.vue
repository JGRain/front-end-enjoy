<template><div><p>分治法求最大子数组的 javascript 实现。</p>
<!--more-->
<h1 id="分治法求最大子数组" tabindex="-1"><a class="header-anchor" href="#分治法求最大子数组" aria-hidden="true">#</a> 分治法求最大子数组</h1>
<h2 id="分治法" tabindex="-1"><a class="header-anchor" href="#分治法" aria-hidden="true">#</a> 分治法</h2>
<p>分治策略递归求解一个问题，在每层递归中：</p>
<ul>
<li>分解(Divide)步骤：将问题划分未一些子问题，子问题的形式与原问题一样，只是规模更小</li>
<li>解决(Conquer)步骤：递归地求解出子问题。如果子问题的规模足够小，则停止递归，直接求解</li>
<li>合并(Combine)步骤：将子问题的解组合成原问题的解</li>
</ul>
<h2 id="最大子数组问题" tabindex="-1"><a class="header-anchor" href="#最大子数组问题" aria-hidden="true">#</a> 最大子数组问题</h2>
<p>求数组的和最大的非空连续子数组，即最大子数组。</p>
<h2 id="分治策略思路" tabindex="-1"><a class="header-anchor" href="#分治策略思路" aria-hidden="true">#</a> 分治策略思路</h2>
<p>将子数组划分为两个规模尽量相等的子数组，<code v-pre>A[low, ..., high]</code>的一个最大子数组<code v-pre>A[i, .., j]</code>所处的位置必然是三种情况之一：</p>
<ul>
<li>完全位于子数组<code v-pre>A[low, ..., mid]</code>中，<code v-pre>low &lt;= i &lt;= j &lt;= mid</code></li>
<li>完全位于子数组<code v-pre>A[mid + 1, ..., high]</code>中，<code v-pre>mid &lt;= i &lt;= j &lt;= high</code></li>
<li>跨越了中点，<code v-pre>low &lt;= i &lt;= mid &lt; j &lt;= high</code></li>
</ul>
<p>可递归求解<code v-pre>A[low, ..., mid]</code>和<code v-pre>A[mid + 1, ..., high]</code>。</p>
<p>求解跨越中点的最大子数组的思路：</p>
<ol>
<li>循环求出<code v-pre>A[i, ..., mid]</code>以及<code v-pre>A[mid + 1, ..., j]</code>的最大和</li>
<li>合并返回左侧和右侧最大和</li>
</ol>
<h2 id="js-实现" tabindex="-1"><a class="header-anchor" href="#js-实现" aria-hidden="true">#</a> js 实现</h2>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">findMaxSubarray</span><span class="token punctuation">(</span><span class="token parameter">iArr<span class="token punctuation">,</span> low<span class="token punctuation">,</span> high</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> inputArr <span class="token operator">=</span> <span class="token punctuation">[</span>
